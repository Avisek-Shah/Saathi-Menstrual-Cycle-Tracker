...
**Decision:** Two independent things, same session. (1) Root-caused the "weird icon on Android, fine on iOS" report: `android.adaptiveIcon.foregroundImage` was pointing at `assets/app-icon-period-tracker.png`, the *whole* flattened icon (its own baked rounded-square backdrop + drop shadow + ~15% transparent canvas padding) rather than a proper adaptive-icon foreground layer (glyph only, transparent elsewhere). Android's mask + `backgroundColor` composited on top of that padded, backdrop-already-drawn image, producing a nested square-inside-a-mask artifact; iOS/legacy just draws the flat square directly, so it looked fine there. (2) Separately, the user supplied an Icon Kitchen export (`IconKitchen-Output.zip`) built from a *different* source image (a two-droplet + "SAATHI" text-banner graphic, not the single-droplet-in-circle icon) — flagged this mismatch and confirmed with the user before applying (rule 1) sourced from the zip and used verbatim, replacing the icon design for both platforms: `assets/app-icon-period-tracker.png` (used for `expo.icon`, i.e. iOS + fallback) now the zip's `ios/AppIcon~ios-marketing.png`; two new files, `assets/android-icon-foreground.png` and `assets/android-icon-monochrome.png`, are the zip's `ic_launcher_foreground.png`/`ic_launcher_monochrome.png` (proper transparent-margin glyph layers). `android.adaptiveIcon` now sets `foregroundImage` + `monochromeImage` to those two files and keeps `backgroundColor: #FFFFFF` (the zip's background layer is flat white, so a colour is simpler than a redundant image). `npx expo prebuild --platform android` was run to regenerate the gitignored `android/` project against the new config; confirmed the regenerated `ic_launcher_foreground.webp`/`ic_launcher.webp` no longer show the nested-square artifact.
**Reason:** User reported the launcher icon rendering incorrectly on Android only, then supplied the Icon Kitchen zip and, once the source-image mismatch was flagged, explicitly chose "use zip as-is" over fixing the original design or asking for a re-export.
**Reversible?** yes — `assets/app-icon-period-tracker.png` (old version) and the previous `adaptiveIcon` config are in git history; the new `assets/android-icon-*.png` files are additive.

## 2026-09-06 — Ring centre hero shrinks when eyebrow + 2-line hero + date line + chip all stack

**Spec section:** unspecified (§2.3 fixes ring diameter at 240dp and typography.hero at 34px, but not what happens when every optional centre line is present at once)
**Decision:** `CycleRingCard.tsx` adds a `crowded` flag (`centre.dateLine && centre.chip` both present — the fertile/ovulation/menstruating/postPeriod/expectedNow/luteal phases with a confidence chip). Only when crowded, the hero `Text` renders at 22px/26 line-height instead of the spec's 34px; the chip pill also gets a `maxWidth: 168` so its single-line (see the 2026-09-06 chip-wrap entry above) text can't stretch wider than the ring's usable width. Every phase without both a date line and a chip keeps the spec's 34px hero.
**Reason:** User reported (screenshot) the confidence chip still visually overlapping the ring band even after the chip was forced to one line — the real cause was total stack height (eyebrow + a wrapped 2-line hero at full 34px + date line + chip), not chip wrapping alone. `typography.hero` at 34px was sized for the common single-line case; a 2-line "Fertile window · day 7 of 7" at that size plus three more stacked lines exceeds the vertical room inside a 240dp ring before the band strokes start.
**Reversible?** yes — one boolean and two style values in `CycleRingCard.tsx`; no shared token or other screen touched.

## 2026-09-06 — Today's date-of-month drawn inside the ring's today-marker (deviation from §2.3)

**Spec section:** UI/UX spec §2.3 ("No numerals on the rim") — deviation, one numeral only
**Decision:** The today-marker halo (`CycleRing.tsx`) now shows today's date-of-month (`formatDate(today, calendarSystem, 'd')`, computed in `CycleRingCard.tsx` and passed down as `todayLabel`) instead of a plain filled dot. No other numerals are added anywhere else on the rim — the four phase-boundary ticks and the rest of the ring are unchanged. Separately, the confidence chip on the ring's centre (`ringChipFromYou` / `confidenceEstimate`) was overflowing onto the ring band when it wrapped to two lines (e.g. "estimate — from what you told us" alongside a two-line fertile-window hero); the chip `Text` now takes `numberOfLines={1}` with `adjustsFontSizeToFit`/`minimumFontScale={0.75}`, matching the pattern already used on the hero line, so it shrinks instead of wrapping.
**Reason:** §2.3's "no numerals on the rim" was written to kill a 30-numeral rim (one per cycle day) that was the root cause of a prior ambiguity bug — see the 2026-09-06 "Home ring reverted to cycle-relative, numerals removed" entry below. A single always-present today digit is a different shape of information (it answers "what's today's date", not "which cycle day is this") and was requested explicitly by the user after this exact conflict was flagged and confirmed.
**Reversible?** yes — `todayLabel` is an optional prop; passing `null`/omitting it restores the plain dot marker.
...
## 2026-09-06 — Android adaptive icon relayered; icon assets regenerated

**Spec section:** none (REQUIREMENTS.md has no icon/splash visual-identity section)
**Decision:** `assets/app-icon-period-tracker.png` was being used simultaneously as `expo.icon` and as `android.adaptiveIcon.foregroundImage`, but it is a *complete* icon — a peach plate with a 203px circular hole punched through it and transparent padding around it — not a foreground layer. Android filled every transparent region with `adaptiveIcon.backgroundColor` (`#FFFFFF`), producing a white disc inside a white frame around the peach plate, then drew its own elevation shadow around the masked result — the grey border seen on the launch screen. All three icon assets are now regenerated from the plate's own artwork: `app-icon-period-tracker.png` is a full-bleed opaque 1024px peach square (no transparency, so iOS cannot mask a hole into it), `android-icon-foreground.png` is a 432px transparent layer holding only the white disc and droplet inside the 288px safe zone, and `android-icon-monochrome.png` is the droplet silhouette alone. `adaptiveIcon.backgroundColor` changed `#FFFFFF` → `#FCE9DA` (the plate's own peach, sampled from the artwork).
**Reason:** User reported a grey border framing the icon on Android launch. Pixel inspection of the asset ruled out a baked-in shadow — the artwork is clean; the fault was the layering. Splash config was also reverted to the previously shipped `assets/splash-icon.png` @ 160 because the new full-bleed peach icon would show a visible seam against the `#f8e4cd` splash background.
**Reversible?** yes — assets and config only; prior versions are in git history. Requires a new native build to take effect, as icon/splash resources do not ship over EAS Update.

## 2026-09-06 — Committed `android/` made the icon config dead; native resources regenerated, edge compositing fixed

**Spec section:** none (REQUIREMENTS.md has no icon/splash visual-identity section)
**Decision:** Commit `e98ac55` added the generated `android/` directory to version control and dropped `/android` from `.gitignore`, moving the project from Expo's managed prebuild (CNG) to the bare workflow. In that workflow EAS Build ships the committed `android/` verbatim and never runs prebuild, so the previous entry's `app.json` fix — including `adaptiveIcon.backgroundColor: #FCE9DA` — was never reaching a build; the committed `values/colors.xml` still carried `iconBackground` `#FFFFFF`, and the committed mipmaps still held the wrong artwork. `npx expo prebuild -p android` was re-run to bring the tracked native resources back in step with `app.json`. Separately, the icon-layer generator was compositing onto a transparent canvas with premultiplied rather than straight alpha, leaving a dark hairline (`rgb ≈ alpha`) around the white disc in `android-icon-foreground.png`; and `assets/splash-icon.png` carried a ~2px neutral-grey stroke (`rgb(219,215,213)`) baked around its circle. Both edges were rebuilt: all three assets now measure zero neutral-grey pixels, and the foreground composited over `#FCE9DA` transitions white → peach with no dark dip.
**Reason:** User reported the grey border was still present after the previous fix. The tracked `android/` directory, not the artwork, was why nothing changed on device. `android/` is deliberately left committed — that is the user's choice from `e98ac55` — but it now means `app.json` icon, splash and manifest changes require `npx expo prebuild -p android` to be re-run and committed, or they will silently not ship.
**Reversible?** yes — assets and generated native resources only; prior versions are in git history. Still requires a new native build: icon and splash resources never ship over EAS Update.
